...
public class Model {
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int i = 0; i < board.size() - 1; ++i) {
            for (int j = 0; j < board.size() -1; ++j) {
                if (board.tile(i,j).value() == board.tile(i,j+1).value() || board.tile(i,j).value() == board.tile(i+1,j).value() || board.tile(i+1,j).value() == board.tile(i+1,j+1).value() || board.tile(i,j+1).value() == board.tile(i+1,j+1).value() ) {
                    return true;
                }
            }
        }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        int myValue = board.tile(x,y).value();
        int targetY = y;
        boolean merged = false;

        for (int newY = y + 1; newY < board.size(); newY++) {
            Tile tileAbove = board.tile(x, newY);

            if (tileAbove == null) {
                targetY = newY;
            }
            else if (tileAbove.value() == currTile.value() && !tileAbove.wasMerged()) {
                targetY = newY;
                merged = true;
                break;
            }
